...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        # aggregate it later
        ### YOUR CODE HERE ###
        el =  torch.abs(self.iql_expectile - (diff <= 0).float()) * torch.square(diff)
        #el = self.mse_loss((diff <= 0).float(), self.iql_expectile)
        return el
        ### YOUR CODE HERE ###
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE HERE ###
        q = self.q_net_target(ob_no).gather(1, ac_na.type(torch.int64).unsqueeze(1))
        v = self.v_net(ob_no) 
        diff = q - v
        value_loss = torch.sum(self.expectile_loss(diff))
        ### YOUR CODE HERE ###
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ### YOUR CODE HERE ###
        Q = self.q_net(ob_no).gather(1, ac_na.unsqueeze(1))
        v = self.v_net(next_ob_no)
        #         print(self.gamma * v) 
        rv =  reward_n.unsqueeze(1)  + (1 - terminal_n.unsqueeze(1)) *self.gamma * v
        loss = self.mse_loss(Q,rv)
        ### YOUR CODE HERE ###
